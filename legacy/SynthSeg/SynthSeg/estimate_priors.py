...
def sample_intensity_stats_from_single_dataset(image_dir, labels_dir, labels_list, classes_list=None, max_channel=3,
    ...
    for idx, (path_im, path_la) in enumerate(zip(path_images, path_labels)):
        ...
        la, aff_la, _ = utils.load_volume(path_la, im_only=False)
        image, aff_im, _ = utils.load_volume(path_im, im_only=False)

        # first align orientation only, so both are in a consistent axis convention
        n_dims, _ = utils.get_dims(list(image.shape), max_channels=10)
        image, aff_im = edit_volumes.align_volume_to_ref(image, aff_im, return_aff=True, n_dims=n_dims)
        la, aff_la = edit_volumes.align_volume_to_ref(la, aff_la, aff_ref=aff_im, return_aff=True, n_dims=n_dims)

        # then resample LABELS to IMAGE grid
        la = edit_volumes.resample_volume_like(
            vol_ref=image,
            aff_ref=aff_im,
            vol_flo=la,
            aff_flo=aff_la,
